...
# on remarque grâce à la courbe que fibo_4 prend beaucoup moins de temps pour atteindre la même valeur de fibonacci
# avec une asymptote en x**log2(3) alors que fibo_3 était en quadratique
...
# on remarque que fibo_4 a un nombre d'opération qui évolu de manière logarithmique
# tandis que fibo_3 a un nombre d'opération qui évolue de manière linéaire
...
def decompte_karatsuba (a, b, c, d, e) :
  # cout de l'opération a = b*c+d*e lorsque la multiplication d'entiers utilise l'algorithme de Karatsuba
    nb_bits_b = nbOfBits(b)
    nb_bits_c = nbOfBits(c)
    nb_bits_d = nbOfBits(d)
    nb_bits_e = nbOfBits(e)
    karatsuba_cost_bc = (nb_bits_b * nb_bits_c) ** (log2(3) / 2)
    karatsuba_cost_de = (nb_bits_d * nb_bits_e) ** (log2(3) / 2)
    return int(karatsuba_cost_bc + karatsuba_cost_de) 
...
# on remarque sur la prmière courbe que, avec le décompte naïf, la courbe des opérations sur les bits de fibo_4 
# est au dessus de celle de fibo_3, indiquant que pour le même temps, fibo_4 fait moins d'opérations mais reste (pour de grandes valeurs)
# au dessus de la courbe en x**log2(3)
# Avec la multiplication par l'algorithme de karatsuba, la courbe de fibo_4 de rapproche de manière asymptotique 
# de la courbe témoin, indiquant que pour les grandes valeurs, elle fait moins d'opérations sur les bits.
